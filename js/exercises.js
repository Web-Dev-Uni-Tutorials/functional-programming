function hasPassed(mark,successCallback, failureCallback)
{
  if(mark>=40){
    successCallback()
  }else{
...
const consoleMsg = function ()
{
  console.log("Well done, you passed!");
}
const failMsg = function ()
{
  console.log("Sorry, you failed!");
}
const domMsg = function ()
{
  const msgDiv = document.querySelector("#msg");
  msgDiv.textContent = "Well done, you passed!";
}

// hasPassed(50,alertMsg);
// hasPassed(50,consoleMsg);
// hasPassed(50,domMsg);
...
function doItTwice(callback){
  callback();
  callback();
}

doItTwice(consoleMsg); //outputs "Well done, you passed!" twice to the console
...
const printInConsole = function(films){
...
function showInfo(film){
  return function(){
    ...
    infoDiv.textContent = `The film ${film.title} was released in ${film.year}.`
  }
}
...
const printInDOM = function(films){
  ...
  films.forEach(function(film){
    ...
    newParagraph.addEventListener("click",showInfo(film),false);
...
filterFilms(printInConsole)
filterFilms(printInDOM)
...
*/
function getMultiplyFunction(multiplier){
	return function(num)
	{
...
/*
5. This question is about returning functions and closures. You should have an example where you have looped over the array of film objects and displayed them in the HTML page.
a) Modify this example so that each film title in the page is clickable, i.e. listen for a click event on each film title. Create an event handler function (this should simply the message "you clicked on a film" in the console) that will be called when any one of the film titles is clicked.
